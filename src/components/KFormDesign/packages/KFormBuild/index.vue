<template>
  <a-config-provider :locale="locale">
    <a-form v-if=" !!value.list && !!value.config" class="k-form-build-9136076486841527" :layout="value.config.layout" :hideRequiredMark="value.config.hideRequiredMark" :form="form" @submit="handleSubmit" :style="value.config.customStyle">
      <buildBlocks ref="buildBlocks" @handleReset="reset" v-for="(record, index) in value.list" :record="record" :dynamicData="dynamicData" :config="config" :disabled="disabled" :formConfig="value.config" :key="index" @change="handleChange" />
    </a-form>
  </a-config-provider>
</template>
        ...
        try {
          this.form.validateFields((err, values) => {
            ...
            this.$refs.buildBlocks.forEach(item => {
              if (!item.validationSubform()) reject(err)
            })
...
.k-form-build-9136076486841527 /deep/ .ant-card + .ant-card {
  margin-top: 12px;
}
</style>
