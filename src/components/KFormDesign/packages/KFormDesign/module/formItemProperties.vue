<template>
  <div class="properties-centent kk-checkbox">
    ...
    <div class="properties-body">
      ...
      <a-form v-show="selectItem.key !== ''">
        ...
        <a-form-item v-if="selectItem.type === 'grid'" label="列配置项">
          ...
          <KChangeOption v-model="selectItem.columns" type="responsive" />
        </a-form-item>
        ...
        <a-form-item v-if="selectItem.type === 'date'" label="默认值">
          ...
          <a-input v-if="options.range" v-model="options.rangeDefaultValue[0]" :placeholder="typeof options.format === 'undefined' ? '' : options.format" />
          <a-input v-if="options.range" v-model="options.rangeDefaultValue[1]" :placeholder="typeof options.format === 'undefined' ? '' : options.format" />
        </a-form-item>
        <!-- 日期选择器默认值 start -->
        <a-form-item v-if="![
          'number',
          ...
          'switch',
          'slider',
          'html'].includes(selectItem.type) && typeof options.defaultValue !== 'undefined'" label="默认值">
          <a-input v-model="options.defaultValue" :placeholder="typeof options.format === 'undefined' ? '请输入' : options.format" />
        </a-form-item>
        <!-- 修改html -->
        <a-form-item v-if="selectItem.type === 'card'" label="是否含有边框">
          <a-switch v-model="options.bordered" />
        ...
        <a-form-item v-if="typeof options.hidden !== 'undefined' ||
          ...
          typeof options.showInput !== 'undefined'" label="操作属性">
        ...
        <!-- input其他验证项 -->
        <!-- <a-form-item v-if="typeof selectItem.rules !== 'undefined' && selectItem.rules.length > 0 && selectItem.type === 'input'">
          <kCheckbox v-model="selectItem.rules[1].enable" label="必须中文" />
          <a-input v-model="selectItem.rules[0].message" placeholder="必填校验提示信息" />
          <KChangeOption v-model="selectItem.rules" type="rules" />
        </a-form-item> -->
    ...
    <div class="close-box" @click="handleHide">
